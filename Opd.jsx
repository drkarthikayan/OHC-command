...
const SHIFT_META = {
  General:   { color:'text-blue-400 bg-blue-400/10',    icon:'\u2600\ufe0f' },
  Morning:   { color:'text-amber-400 bg-amber-400/10',  icon:'\U0001f305' },
  Afternoon: { color:'text-orange-400 bg-orange-400/10',icon:'\U0001f324\ufe0f' },
  Night:     { color:'text-indigo-400 bg-indigo-400/10',icon:'\U0001f319' },
};
      ...
      <div className="card overflow-hidden">
        ...
        {loading ? (
          ...
          <div key={v.id} className="grid grid-cols-[2fr_2fr_80px_80px_80px_1fr_80px] gap-2 px-4 py-3 border-b border-border/50 last:border-0 hover:bg-surface2/30 transition-colors items-center">
            ...
            <div>{(() => { const m=SHIFT_META[v.shift||'General']; return <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full ${m.color}`}>{m.icon} {v.shift||'General'}</span>; })()}</div>
        ...
        <div className="modal-backdrop" onClick={e=>e.target===e.currentTarget&&setViewing(null)}>
          <div className="modal-box w-full max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            <div className="modal-body space-y-4">
              ...
              {viewing.shift && (() => { const m=SHIFT_META[viewing.shift||'General']; return <div className="flex items-center gap-2"><span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${m.color}`}>{m.icon} {viewing.shift} Shift</span></div>; })()}
