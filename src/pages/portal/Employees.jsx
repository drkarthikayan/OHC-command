...
export default function EmployeesPage() {
  ...
  return (
    <div className="p-6 max-w-6xl">
        ...
        {loading
          ...
          : filtered.length === 0
            ? <div className="py-12 text-center">
                ...
                <div className="text-muted text-sm">{employees.length === 0 ? 'No employees yet. Add your first employee to get started.' : 'No results match your search.'}</div>
                {employees.length === 0 && canEdit && <button onClick={openAdd} className="btn-primary mt-4 text-sm">+ Add First Employee</button>}
              </div>
            : filtered.map(emp => (
              <div key={emp.id} className="grid grid-cols-[2fr_1fr_1fr_1fr_80px_80px] gap-2 px-4 py-3 border-b border-border/50 last:border-0 hover:bg-surface2/30 transition-colors items-center">
                <div className="flex items-center gap-2.5 min-w-0">
                  ...
                  <div className="min-w-0">
                    <button
                      ...
                      className="text-sm font-medium text-text hover:text-accent truncate block text-left group"
                      title="Click to view health profile"
                    >
                      {emp.name}
                      <span className="opacity-0 group-hover:opacity-100 ml-1 text-xs text-accent transition-opacity">→</span>
                    </button>
                ...
                <div className="flex gap-1">
                  {canEdit&&<button onClick={()=>openEdit(emp)} className="text-muted hover:text-accent text-sm p-1" title="Edit">✏️</button>}
                  {canDelete&&<button onClick={()=>setDeleteTarget(emp)} className="text-muted hover:text-red-400 text-sm p-1" title="Delete">🗑️</button>}
                </div>
              </div>
            ))
        }
      ...
      {/* 360° Health Profile Drawer */}
      {profileEmp && (
        <EmployeeProfile
          emp={profileEmp}
          tid={tid}
          onClose={() => setProfileEmp(null)}
          onEdit={canEdit ? () => { setProfileEmp(null); openEdit(profileEmp); } : null}
        />
      )}
