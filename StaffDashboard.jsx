        ...
        <div className="space-y-4">
          ...
          <div className="card overflow-hidden">
            ...
            <div className="p-3 grid grid-cols-2 gap-2">
              {[
                {label:'Morning',   icon:'🌅', key:'Morning',   color:'text-amber-400 bg-amber-400/10'},
                {label:'Afternoon', icon:'🌤️', key:'Afternoon', color:'text-orange-400 bg-orange-400/10'},
                {label:'Night',     icon:'🌙', key:'Night',     color:'text-indigo-400 bg-indigo-400/10'},
                {label:'General',   icon:'☀️', key:'General',   color:'text-blue-400 bg-blue-400/10'},
              ].map(s=>(
                <div key={s.key} className={`flex items-center gap-2 px-2.5 py-2 rounded-lg ${shiftBreakdown[s.key]>0?s.color.split(' ')[1]:'bg-surface2'}`}>
                  <span className="text-sm">{s.icon}</span>
                  <div className="min-w-0">
                    <div className={`text-sm font-bold ${shiftBreakdown[s.key]>0?s.color.split(' ')[0]:'text-muted'}`}>{shiftBreakdown[s.key]}</div>
